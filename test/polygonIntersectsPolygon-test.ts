...
import { polygonIntersectsPolygon, Point } from "../";
...
tape("If polygonA in polygonIntersectsPolygon is forced closed, polygonA will not be altered", function(test) {
...
tape("polygonIntersectsPolygon(polygonA, polygonB) returns true if overlapping rectangles share two edges", function(test) {
    ...
    b = [new Point(562.6875, 298.4375), new Point(601.09375, 298.4375), new Point(601.09375, 316.9375), new Point(562.6875, 316.9375)];
...
tape("polygonIntersectsPolygon(polygonA, polygonB) detects intersections in special cases", function(test) {
